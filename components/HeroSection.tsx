...
import { ArrowRight, Sparkles, BookOpen, Award, Zap, Shield, Download } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Card } from '@/components/ui/card'

const features = [
  { icon: Zap, label: 'Instant Access', color: 'from-yellow-500 to-orange-500' },
  { icon: Shield, label: 'Verified Content', color: 'from-green-500 to-emerald-500' },
  { icon: Award, label: 'Expert Curated', color: 'from-blue-500 to-indigo-500' },
]

const popularTopics = [
  { name: 'System Design', count: '120+ PDFs', gradient: 'from-purple-500 to-pink-500' },
  { name: 'Data Structures', count: '85+ PDFs', gradient: 'from-blue-500 to-cyan-500' },
  { name: 'SQL & Databases', count: '95+ PDFs', gradient: 'from-green-500 to-teal-500' },
  { name: 'Algorithms', count: '110+ PDFs', gradient: 'from-orange-500 to-red-500' },
]
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        >
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-foreground mb-2">Browse Popular Topics</h2>
            <p className="text-muted-foreground">Start learning from our most-loved collections</p>
          </div>
              ...
              >
                <Card className="relative h-full p-6 bg-card hover:shadow-xl transition-all duration-300 border-2 border-border hover:border-indigo-200 overflow-hidden group">
                  ...
                  <div className="relative">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${topic.gradient} flex items-center justify-center mb-4 shadow-lg`}>
                      <BookOpen className="w-6 h-6 text-white" />
                    </div>
                    <h3 className="text-lg font-bold text-foreground mb-2">{topic.name}</h3>
                    <p className="text-sm text-muted-foreground font-medium">{topic.count}</p>
        ...
        >
          <div className="inline-flex flex-col sm:flex-row items-center gap-8 bg-gradient-to-r from-indigo-50 to-purple-50 px-8 py-6 rounded-2xl border border-indigo-100">
            <div className="flex items-center gap-3">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-full border-2 border-white bg-gradient-to-br from-indigo-400 to-purple-400 flex items-center justify-center text-white font-bold text-sm"
                  >
                    {i === 3 ? '12K' : ''}
                  </div>
                ))}
            ...
            <div className="h-12 w-px bg-indigo-200 hidden sm:block" />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-sm font-bold text-foreground">50K+ Downloads</p>
